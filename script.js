...
function setupBackgrounds() {
    ...
    change(0);
    let count = 1;
    setInterval(() => change(count++), (cfg.changeInterval || 20) * 1000);
}
...
async function setupTexts() {
    ...
    setInterval(() => {
        ...
        div.style.cssText = `
            ...
            font-size:${Math.random()*12+14}px; color:hsla(${Math.random()*360},70%,80%,0.4); 
